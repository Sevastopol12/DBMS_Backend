...
from ..canonical import CANONICAL_FIELD_SET
from ..identifiers import classify_bhxh
from ..normalization import normalize_datetime, normalize_identifier, normalize_text, normalized_token
...
from ...models.contracts import RuleMode, ValidationPolicy
from .core import (
    ValidationResult,
    ValidationState,
    _canonical_decimal,
...
def _validate_bhyt(
    ...
    if 8 <= len(compact) <= 20 and compact.isalnum():
        result = ValidationResult(
            state=ValidationState.VALID, normalized_value=compact
        )
...
def _validate_gender(
    ...
    if normalized in {"nam", "nu", "male", "female", "m", "f"}:
        ...
        return ValidationResult(
            state=ValidationState.VALID, normalized_value=canonical
        )
    return ValidationResult(
        state=ValidationState.INVALID,
